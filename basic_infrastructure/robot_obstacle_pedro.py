...
#
#  *** MOD v13 (Emergência Ultrassom) ***
#  • Adicionada lógica no loop 'main' para escutar mensagens assíncronas do Arduino
#    (ESTOP, OBSTACLE, RESUME) e gerenciar estados de parada/retomada.
#
...
def enviar_comando_motor_serial(arduino, v_esq, v_dir):
    ...
    # NOTA: Esta função não lê a resposta "OK" ou "OB".
    # A lógica de obstáculo agora é assíncrona.
...
def main():
    ...
    try:
        arduino.flushInput() # Limpa buffer de entrada
        arduino.write(b'A10\n')
    ...
    # Estados: INIT, LEAVING, NAVIGATING, RETURNING, DONE
    # Novos Estados: ESTOP, ESTOP_OBSTACLE
    auto_state = "INIT"  
    last_auto_state_before_stop = "INIT" # Para guardar o estado antes da parada
    ...
    try:
        ...
        while True:  # Loop principal com controle remoto
            ...
            # --- 1. Verificar mensagens do Arduino ---
            arduino_message = None
            if arduino.in_waiting > 0:
                try:
                    # Lê linha do Arduino (espera-se ESTOP, OBSTACLE, ou RESUME)
                    line = arduino.readline().decode('utf-8').strip()
                    if line in ["ESTOP", "OBSTACLE", "RESUME"]:
                        arduino_message = line
                        print(f"📬 Mensagem do Arduino: {arduino_message}")
                except Exception as e:
                    print(f"Erro lendo do Arduino: {e}")

            # --- 2. Verificar comandos remotos ---
            ...
            if remote_key == 'm':
                ...
                if manual_mode:
                    ...
                    last_auto_state_before_stop = "INIT"
            ...
            if manual_mode:
                ...
                # --- 3. Modo Automático: Processar eventos ---
                
                # Processar mensagens do Arduino (maior prioridade)
                if arduino_message == "ESTOP":
                    # Se não estivermos já parados, salvar estado atual
                    if auto_state not in ["ESTOP", "ESTOP_OBSTACLE"]:
                        print(f"🛑 ESTOP recebido! Salvando estado: {auto_state}")
                        last_auto_state_before_stop = auto_state
                        auto_state = "ESTOP"
                        
                elif arduino_message == "OBSTACLE":
                    # Entra no estado de "rotina de obstáculo"
                    if auto_state != "ESTOP_OBSTACLE":
                        print(f"🔄 OBSTACLE recebido! Entrando em rotina de espera.")
                        # Salva estado caso venha de um estado que não seja ESTOP
                        if auto_state != "ESTOP":
                            last_auto_state_before_stop = auto_state
                        auto_state = "ESTOP_OBSTACLE"
                        
                elif arduino_message == "RESUME":
                    # Se estávamos parados, retomar
                    if auto_state in ["ESTOP", "ESTOP_OBSTACLE"]:
                        print(f"✅ RESUME recebido! Retomando estado: {last_auto_state_before_stop}")
                        auto_state = last_auto_state_before_stop
                        
                
                # --- 4. Máquina de Estados Automática ---
                
                # A máquina de estados só roda se não estivermos em um estado de parada
                if auto_state == "INIT":
                ...
                elif auto_state == "ESTOP":
                    # Estado de Parada de Emergência
                    # O RPi não faz NADA. Apenas espera.
                    print("🛑 PARADA DE EMERGENCIA (Aguardando Arduino...)")
                    send_basic_frame(camera, "PARADA DE EMERGENCIA")
                    
                elif auto_state == "ESTOP_OBSTACLE":
                    # Estado de Rotina de Obstáculo
                    # O RPi sabe que o Arduino está dando ré.
                    # No futuro, o RPi poderia usar esse estado para
                    # planejar um desvio assim que receber "RESUME".
                    # Por agora, ele só espera.
                    print("🔄 OBSTACULO (Aguardando Arduino dar re e liberar)")
                    send_basic_frame(camera, "OBSTACULO - AGUARDANDO")

...
# #################### INÍCIO DA MUDANÇA (B) ####################
# Nova função para pedir a distância do ultrassom
...
def main():
    ...
    try:
        ...
        # #################### INÍCIO DA MUDANÇA (A) ####################
        # A task4 (detecção de obstáculo) foi desativada no Arduino.
        # Removemos a linha que a ativava (comando 'I1').
    ...
    try:
        for frame in camera.capture_continuous(raw, format="bgr", use_video_port=True):
            ...
            if current_mode == MODO_AUTO:
                ...
                # #################### INÍCIO DA MUDANÇA (C) ####################
                # 1. Pede a distância ao Arduino
                distancia_atual = get_ultrasonic_distance(arduino)
                
                # 2. Imprime a distância no terminal (conforme solicitado)
                if distancia_atual != -1:
                    print(f"Distancia do Ultrassom: {distancia_atual} cm")
                
                # 3. Lógica de parada (baseada na distância lida)
                #    A verificação 'resposta_arduino == "OB"' foi removida.
                #    Substitua o '15' pelo seu valor desejado (ex: ULTRASONIC_STOP_DISTANCE_CM)
                
                if state != MODO_OBSTACLE_TURN and (distancia_atual > 0 and distancia_atual < 15):
            ...
            # Mantemos esta linha, embora 'resposta_serial' não deva mais ser "OB"

...
def straight_until_seen_then_lost(arduino, camera):
    ...
    try:
        for f in camera.capture_continuous(raw, format="bgr", use_video_port=True):
            ...
            resposta_arduino = drive_cap(arduino, current_speed, current_speed)
	    
	    # Verificamos a resposta do Arduino
            if resposta_arduino == "OB":
                print("!!! 🛑 OBSTÁCULO (reta cega) !!!")
                drive_cap(arduino, 0, 0) # Garante a parada
...
            resposta_arduino = drive_cap(arduino, v_esq, v_dir, cap=ALIGN_CAP)
            ...
            # Verificamos a resposta do Arduino
            if resposta_arduino == "OB":
                print("!!! 🛑 OBSTÁCULO (go_to_next_intersection) !!!")
                drive_cap(arduino, 0, 0) # Garante a parada
                return False # Sinaliza falha/obstáculo para o follow_path
            
            # Se a resposta for "OK" ou "TIMEOUT", o loop continua normalmente
...
def a_star(start,goal,grid=(5,5), blocked_edges_set=set()): # Adicionado blocked_edges_set
    open_set={start}; came={}; g={start:0}; f={start:manhattan(start,goal)}
    W,H=grid
    def neigh(n):
        x,y=n
        cand=[]
        # Verifica se a "aresta" (edge) entre o nó 'n' e o 'vizinho' está bloqueada
        if y-1>=0 and (n,(x,y-1)) not in blocked_edges_set and ((x,y-1),n) not in blocked_edges_set: cand.append((x,y-1))
        if x+1<W  and (n,(x+1,y)) not in blocked_edges_set and ((x+1,y),n) not in blocked_edges_set: cand.append((x+1,y))
        if y+1<H  and (n,(x,y+1)) not in blocked_edges_set and ((x,y+1),n) not in blocked_edges_set: cand.append((x,y+1))
        if x-1>=0 and (n,(x-1,y)) not in blocked_edges_set and ((x-1,y),n) not in blocked_edges_set: cand.append((x-1,y))
        return cand
    while open_set:
...
# [LINHA 1242, antes de def main()]
blocked_edges = set() # Armazena os caminhos bloqueados (ex: ((0,0), (0,1)))

def main():
    global blocked_edges # Adiciona esta linha
    args=parse_args()
...
                        send_basic_frame(camera, "OBSTACULO - Recalculando")
                        ...
                        try:
                            current_path_index = optimized_path.index(cur_node)
                            blocked_node = optimized_path[current_path_index + 1]
                            # Aresta é (nó_atual, nó_bloqueado), ordenados
                            edge = tuple(sorted((cur_node, blocked_node)))
                            blocked_edges.add(edge)
                            print(f"ARESTA BLOQUEADA ADICIONADA: {edge}")
                        except (ValueError, IndexError):
                            print("Erro ao identificar aresta bloqueada. Abortando.")

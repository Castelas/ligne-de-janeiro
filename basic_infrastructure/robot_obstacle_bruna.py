...
# --- AJUSTES DE ESTADO E EVASÃO ---
# Duração da meia-volta em segundos. **VOCÊ PRECISARÁ CALIBRAR ESTE VALOR!**
EVADE_TURN_DURATION = 1.8 
# Velocidade para girar no lugar no estado LOST
SEARCH_SPEED_LOST = 120 
# Frames sem confiança visual até entrar em LOST
LOST_MAX_FRAMES = 5
# Intervalo para verificar parada por obstáculo do Arduino
OBSTACLE_CHECK_INTERVAL = 0.5
# Se a velocidade comandada for > que este valor e a real for 0 -> obstáculo
MIN_SPEED_TO_DETECT_STOP = 50
...
def enviar_comando_serial(arduino, comando):
    arduino.write(comando.encode('utf-8'))
    time.sleep(0.05)
...
def main():
    ...
    try:
     ...
     # Ativa a proteção de obstáculos no Arduino
    enviar_comando_serial(arduino, "I1\n")
    print("Protecao de obstaculos ativada no Arduino.")
    ...
    try:
        for frame in camera.capture_continuous(raw, format="bgr", use_video_port=True):
            ...
            if current_mode == MODO_AUTO:
                ...
                if conf == 1:
                    last_known_error = erro

                    # ##############   MÁQUINA DE ESTADOS CORRIGIDA AQUI   ################
                ...
                if state == 'FOLLOW':
                    ...
                    if conf == 1:
                        ...
                        if lost_frames >= LOST_MAX_FRAMES:
                            state = 'LOST'
                            v_esq, v_dir = 0, 0 # Para antes de procurar

                    # Verificação de obstáculo físico
                    if now - last_obstacle_check_time > OBSTACLE_CHECK_INTERVAL:
                        last_obstacle_check_time = now
                        if abs(v_esq) > MIN_SPEED_TO_DETECT_STOP or abs(v_dir) > MIN_SPEED_TO_DETECT_STOP:
                                # Apenas verifica se deveria estar se movendo
                            resposta_T = enviar_comando_serial(arduino, "T\n")
                            try:
                                real_v_esq, real_v_dir = map(int, resposta_T.split())
                                if real_v_esq == 0 and real_v_dir == 0:
                                    print("!!! OBSTACULO DETECTADO! Entrando no estado EVADING. !!!")
                                    state = 'EVADING'
                                    evade_timer_start = now # Inicia cronômetro da manobra
                                    v_esq, v_dir = 0, 0
                            except (ValueError, IndexError):
                                pass
                    
                # ESTADO: EVADING (Fazendo meia-volta)
                elif state == 'EVADING':
                    v_esq, v_dir = VELOCIDADE_CURVA, -VELOCIDADE_CURVA
                    if now - evade_timer_start > EVADE_TURN_DURATION:
                        print("Evasao concluida. Entrando no estado LOST para reencontrar a linha.")
                        enviar_comando_serial(arduino, "I1\n") # Limpa a flag de obstáculo
                        state = 'LOST'
                        v_esq, v_dir = 0, 0

                # ESTADO: LOST (Procurando a linha)
                elif state == 'LOST':
                    if conf == 1: # Se encontrou a linha
                        print("Linha reencontrada! Voltando ao estado FOLLOW.")
                        state = 'FOLLOW'
                    else: # Se continua perdido, executa a busca
                        # Gira para o lado do último erro conhecido
                        turn_direction = 1 if last_known_error >= 0 else -1
                        v_esq = int(SEARCH_SPEED_LOST * turn_direction)
                        v_dir = int(-SEARCH_SPEED_LOST * turn_direction)

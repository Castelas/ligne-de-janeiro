...
int obstacle_confirmation_count = 0;
#define OBSTACLE_CONFIRMATION_LIMIT 3 // Precisa de 3 leituras (300ms) para forçar parada
...
int del4 = 500;  // tache de détection des obstacles
...
int MeasureDistance(){               // a low pull on pin COMP/TRIG  triggering a sensor reading
  ...
  if (duration == 0) return 999; // Retorna um valor alto em caso de falha
  return duration / 58;
}
...
inline void task4() {
  if (((int)millis() - tim4) > 0) {
    int current_distance = MeasureDistance();
    if (current_distance < 900) {
        distance = current_distance; // Atualiza a variável global apenas com leituras válidas
    }

    if (distance > 0 && distance < ULTRASONIC_STOP_DISTANCE_CM) {
      obstacle_confirmation_count++;
    } else {
      obstacle_confirmation_count = 0;
      obst = false;
    }

    if (obstacle_confirmation_count >= OBSTACLE_CONFIRMATION_LIMIT) {
      obst = true;
      set_motor1(0);
      set_motor2(0);
    }

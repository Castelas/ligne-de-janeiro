...
bool task2on=true ;
// lancement de la tache 2 de calcul de vitesse
bool task3on=false ;
...
char c,CharIn,m;
bool ConnOn ;
// indique si la carte est connectée
...
char retstring[96] ;
// chaine de retour de commande

// variables de la gestion des moteurs 
long int volatile CountIncr1,CountIncr2 =0 ;
// valeur des compteurs incrémentaux en 32 bits
int nivM1,nivM2 ;         // tension appliquée au moteur
int nivE1,nivE2 ;
// tension de consigne à atteindre pour le démarrage en douceur
int nivM ;                // pointeur sur ces variables
char motA,motB ;
// pointeur sur les moteurs
...
tim4 = (int)millis()+del4;
  ...
  attachInterrupt(digitalPinToInterrupt(ENCODER1B), IntIncrem1, FALLING);
// attache l'interruption à l'encodeur 1
  attachInterrupt(digitalPinToInterrupt(ENCODER2B), IntIncrem2, FALLING);
// attache l'interruption à l'encodeur 2
}
...
if (CharIn=='A') CONNECT_code();        // demande de connection
    else if (ConnOn)     decod_serial(CharIn);
// on ne fait un decodage de commande que si on est connecté
  }

  // lancement des differentes taches périodiques
  if (task1on) task1() ;
// tache periodique non définie
  if (task2on) task2() ;
// tache de calcul de la vitesse moteur toujours en route
  if (task3on) task3() ;
...
// on recupere la commande du moteur
      else
        return(def) ;
...
void RESETENC_code() {
    ...
    CountIncr2=0;
    RetAcquitSimpl();
// acquitement de la commande en mode feedback=1
    if (feedback==2)
...
// indispensable et pas trop long
  ...
  if ((m=='1') || (m=='2'))
  { nivM=GetInt(0) ;
// recupere la valeur de la commande
    GetInt(0) ;
    GetLong(0);
    set_motor(motA,motB,s,nivM) ;
...
// indispensable et pas trop long
  ...
  nivM1=GetInt(0) ;
// on lit la valeur du premier moteur
  nivM2=GetInt(nivM1) ;
// on lit la valeur du deuxième moteur
  GetLong(0);
...
// indispensable et pas trop long
  ...
  servopos=GetInt(0) ;
// on recupere la vitesse pour positionner le moteur
  GetInt(0);
  GetLong(0);
...
inline void task3() {
  if (((int)millis()-tim3)>0)  // si il s'est passé 1 seconde depuis la dernière lecture
  {
    if (nivM1 < nivE1)
    {  nivM1 += vitdem ;
// accélération du moteur 1
      if (nivM1> nivE1)        nivM1 = nivE1 ;
}
    if (nivM1 > nivE1)
    {  nivM1 -= vitdem ;
// décélération du moteur 1
      if (nivM1 < nivE1)        nivM1 = nivE1 ;
}
...
void IntIncrem1() {                // interruption du décodeur incrémental
  int sns = (digitalRead(ENCODER1A) == LOW) ?
+1 : -1 ;     // on détermine le sens de rotation
  CountIncr1 += sns;
...
void IntIncrem2() {                // interruption du décodeur incrémental
  int sns = (digitalRead(ENCODER2A) == LOW) ?
-1 : +1 ;     // on détermine le sens de rotation
